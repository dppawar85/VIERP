...
<div class="container" style="position : relative;font-size: 15px; margin-left: 20%">
    <g:form controller="CourseCategory" action="savechangesubcategorylinking" >
                    <lable>Category:</lable>
                    <g:select class="form-control" name="maincategory" from="${maincategorylist.name}"/>
                    <lable>Sub-Category:</lable>
                    <g:select class="form-control" name="subcategory" from="${subcategorylist.name}"/>
                    <g:submitToRemote class="btn btn-primary" url="[controller:'CourseCategory', action:'savechangesubcategorylinking']" update="mainorgranizationajax" value="Change Sub-Category Linking" />
    </g:form>
    <br><br>
